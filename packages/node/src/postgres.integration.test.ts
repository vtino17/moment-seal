...
describe.runIf(enabled)("PostgreSQL optimistic concurrency integration", () => {
  beforeAll(async () => {
    ...
    await clients[0]!.query("INSERT INTO momentseal_integration_versions (id, version, status) VALUES ($1, $2, $3)", ["invoice-204", 1, "pending"]);
  });

  afterAll(async () => {
    if (clients[0]) await clients[0].query("DROP TABLE IF EXISTS momentseal_integration_versions");
  ...
  it("allows exactly one of two competing versioned writes", async () => {
    ...
    const persisted = await clients[0]!.query<{ version: number; status: string }>("SELECT version, status FROM momentseal_integration_versions WHERE id = $1", ["invoice-204"]);
