...
import { generateSigningKeyPair, keyIdFromPublicKey, signReceipt, verifySignedReceipt } from "./signature.js";
...
describe("Ed25519 receipt envelopes", () => {
  ...
  it("supports explicitly unencrypted development keys", async () => {
    ...
    const envelope = await signReceipt({ receipt, privateKey: keys.privateKey, signedAt: new Date("2026-07-29T03:11:00.000Z") });
  ...
  it("fails closed for malformed or inaccessible keys", async () => {
    ...
    await expect(signReceipt({ receipt, privateKey: "not-a-key" })).rejects.toMatchObject({ code: "KEY_INVALID" });
    await expect(signReceipt({ receipt, privateKey: generateSigningKeyPair("right-passphrase").privateKey, passphrase: "wrong-passphrase" })).rejects.toMatchObject({ code: "KEY_INVALID" });
  ...
  it("refuses to sign an invalid receipt hash", async () => {
    ...
    await expect(signReceipt({ receipt: { ...receipt, receiptHash: "0".repeat(64) }, privateKey: keys.privateKey, passphrase: "test-passphrase" })).rejects.toMatchObject({ code: "RECEIPT_SIGNATURE_INVALID" });
  });

  it("refuses signature timestamps before receipt issuance", async () => {
    const { keys, receipt } = await fixture();
    await expect(signReceipt({ receipt, privateKey: keys.privateKey, passphrase: "test-passphrase", signedAt: new Date("2026-07-29T03:09:30.000Z") })).rejects.toMatchObject({ code: "SIGNED_ENVELOPE_INVALID" });
  });
