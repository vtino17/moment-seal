...
const normalize = (value: unknown, ancestors: Set<object>, depth: number): unknown => {
  ...
  if (typeof value !== "object") throw new Error(`Canonical JSON cannot contain ${typeof value} values.`);
  if (ancestors.has(value)) throw new Error("Canonical JSON cannot contain cyclic references.");
  ancestors.add(value);
  try {
    if (Array.isArray(value)) {
      if (Object.keys(value).length !== value.length) throw new Error("Canonical JSON cannot contain sparse arrays.");
    ...
    return Object.fromEntries(Object.entries(value).sort(([a], [b]) => a.localeCompare(b)).map(([key, child]) => {
      if (child === undefined) throw new Error(`Canonical JSON field "${key}" is undefined.`);
