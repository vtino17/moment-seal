...
export function formatCompilation(result: MomentCompilation): string {
  const lines = [
    ...
    `Fresh evidence ${percent(result.metrics.freshObservationCoverage)} · Fresh commit state ${percent(result.metrics.freshCommitStateCoverage)} · Scoped ${percent(result.metrics.scopeBindingCoverage)}`,
    `Conditional mutations ${percent(result.metrics.conditionalCoverage)} · Max gap ${duration(result.metrics.maximumCheckUseGapMs)}`,
    "",
  ];
...
export function formatDot(result: MomentCompilation): string {
  ...
  for (const state of result.graph.states) lines.push(`  ${quote(`state:${state.id}`)} [label=${quote(`${state.id}\\n${state.resourceId} @ ${state.currentVersion}`)}, color="#c4b5fd"];`);
  ...
  for (const action of result.graph.actions) if (action.commitStateId) lines.push(`  ${quote(`state:${action.commitStateId}`)} -> ${quote(`action:${action.id}`)} [label="commit state", style=dashed];`);
