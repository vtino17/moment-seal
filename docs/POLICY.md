...
| `maximumCommitStateAgeMs` | Maximum interval between commit-state capture and action |
...
| `requireScopeBinding` | Bind observation, state, action, lease, and revalidation scope |
| `maximumDependencyDepth` | Maximum longest path in the action graph |
| `maximumActions` | Maximum actions accepted in one plan |
| `maximumObservations` | Maximum observations accepted in one plan |
| `maximumDependencyEdges` | Maximum dependency edges accepted in one plan |

## Suggested profiles

These are starting points, not universal security levels.

| Profile | Observation / gap | Commit state | Revalidation | Authority | Depth |
| --- | ---: | ---: | ---: | ---: | ---: |
| Advisory read-only | 15 min | 2 min | 5 min | 60 | 8 |
| Standard automation | 5 min | 30 sec | 60 sec | 80 | 4 |
| Financial / destructive | 30 sec | 2 sec | 5 sec | 95 | 2 |
